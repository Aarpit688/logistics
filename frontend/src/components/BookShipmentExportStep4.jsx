...
export default function BookShipmentExportStep4({
  data,
  onChange,
  onNext,
  onBack,
    ...
    <>
      <div className="mb-5">
        <h3 className="text-base font-extrabold text-black tracking-wide">
          Select Vendor Price (Export)
        </h3>
        <p className="mt-1 text-xs font-bold text-gray-500">
          Choose one vendor rate to book the export shipment.
        </p>
      </div>

      {rates.length === 0 ? (
        <div className="rounded-md border border-black/10 bg-white p-5 text-sm font-extrabold text-gray-700">
          No vendor rates available for this export shipment.
        </div>
      ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-3 text-xs font-bold text-gray-600">
                      ...
                      {/* ✅ Optional: Service / Carrier info if backend provides */}
                      {rate.serviceType ? (
                        <span className="rounded-md bg-gray-100 px-2 py-1 text-[11px] font-extrabold text-gray-800">
                          {rate.serviceType}
                        </span>
                      ) : null}
                    </div>
